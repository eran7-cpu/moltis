...
async fn share_page_handler(
    ...
    // Validate share_id is a UUID to prevent path-traversal.
    if uuid::Uuid::parse_str(&share_id).is_err() {
        return not_found_share_response();
    }

    // Try serving a pre-rendered static file first (public shares only).
    ...
    // Fallback: dynamic rendering from the database.
...
/// Build the standard share-page response with security headers.
#[cfg(feature = "web-ui")]
...
async fn share_social_image_handler(
    ...
    // Try serving a pre-rendered static SVG first (public shares only).
    let static_path = moltis_config::data_dir()
        .join("shares")
...
/// Build the standard share OG SVG response with security headers.
#[cfg(feature = "web-ui")]
